...
static const oidcWellknownT noWellknown={};
...
// defaults pamOpts
static pamOptsT pamOpts = {
	.gidsMax= 10,
	.avatarAlias= "/sgate/pam/avatar.png"
};
...
static int pamAccessToken (afb_hreq *hreq, oidcIdpT *idp, const oidcProfilsT *profil, const char *login, const char *passwd) {
	...
	gid_t groups[pamOpts.gidsMax];
	int ngroups= pamOpts.gidsMax;
	...
	if (err) {
		EXT_CRITICAL ("[pam-auth-gids] opts{'gids':%d} too small", pamOpts.gidsMax);
		goto OnErrorExit;
	}
	...
	fedUser->avatar= strdup (pamOpts.avatarAlias);
...
int pamInitCB (oidcIdpT *idp, json_object *idpJ, idpGenericCbT *idpConfigCbs) {
	...
	if (pluginJ) {
		err= wrap_json_unpack (pluginJ, "{s?i s?s}"
			,"gids", &pamOpts.gidsMax
			,"avatar", &pamOpts.avatarAlias
		);
		if (err) {
			EXT_ERROR ("[pam-config-opts] json parse fail 'plugin':{'gids': %d, 'avatar':'%s'", pamOpts.gidsMax, pamOpts.avatarAlias);
			goto OnErrorExit;
		}
	}
